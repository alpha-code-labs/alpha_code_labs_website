...
const page = () => {
    ...
    const deetTechData = deepTech.filter((ele) => { return ele.category == 'Deep Tech Studio' });
    // console.log(deetTechData);

    const blogHandler=(id)=>{
    router.push(`/deeptechdetailedblog/${id}`)
    }
    return (
        <>
            <Header />
            <Grid container>
                <Grid item xs={12} sx={{p:'30px',display:'flex', justifyContent:'center', alignItems:'center'}}>
                    <Typography sx={{fontSize:{lg:'40px', md:'35px', sm:'30px', xs:'25px'}, color:'white', fontFamily:'sans-serif',fontWeight:'bold', }}>Deep Tech Blogs</Typography>
                </Grid>
            </Grid>

            {
                deetTechData.length == 0 ?
                    ...
                    <Grid container sx={{ justifyContent: 'space-evenly', alignItems: 'center', p: '30px 0px 60px 0px' }}>
                        {
                            deetTechData.map((ele, index) => {
                                return (
                                    <Grid key={index} item lg={3.1} md={5} sm={10} xs={10} sx={{ backgroundColor: 'rgba(0,0,0,0.5)', borderRadius: '10px', boxShadow: '1px 1px 14px -1px rgba(200,200,200,1)', mt: '8px',mb: '25px',cursor:'pointer' }} onClick={()=>{blogHandler(ele._id)}}>
                                        ...
                                        <Box sx={{ p: '10px' }}>
                                            <Typography sx={{ color: '#b71c1c', fontSize: '20px', fontFamily: 'sans-serif', mt: '10px' }}>Blog Article</Typography>
                                            {/* <Typography sx={{ color: 'white', fontSize: '20px', fontFamily: 'sans-serif', mt: '10px' }}>{ele.category}</Typography> */}
                                            <Typography sx={{ color: '#C7C7C7', fontSize: '17px', fontFamily: 'sans-serif', mt: '10px' }}>{ele.description}</Typography>
                                        </Box>
