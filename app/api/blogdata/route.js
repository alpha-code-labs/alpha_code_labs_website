...
export const POST = async (req) => {
    ...
    try {
        ...
        if(image){
            ...
            const toSave = await blogData(allBlogData);
            // console.log('all data',toSave)
            const saved = await toSave.save();
            // console.log('saved', saved);
            if (saved) {
                return NextResponse.json({ message: 'Blog Add Successfully',resp:saved }, { status: 200 });
            } else {
